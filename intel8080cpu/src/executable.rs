...
impl<'a> Cpu<Intel8080Instruction, CpuError> for Intel8080Cpu<'a> {
    fn execute_instruction(&mut self, instruction: &Intel8080Instruction) -> Result<(), Error> {
        ...
        match *instruction {
            ...
            Intel8080Instruction::Jnz { address } => self.execute_jnz(address[1], address[0]),
            Intel8080Instruction::Jmp { address } => self.execute_jmp(address[1], address[0]),
            Intel8080Instruction::Jp { address } => self.execute_jp(address[1], address[0]),
            Intel8080Instruction::Jpe { address } => self.execute_jpe(address[1], address[0]),
