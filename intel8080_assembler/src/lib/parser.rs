...
impl Parser {
    ...
    fn parse_statement(&mut self, input: &AssemblerToken) -> Result<(), Error> {
        ...
        let expression = match (input, next) {
            ...
            (
                AssemblerToken {  token_type: AssemblerTokenType::LabelToken(ref label), line: _  },
                Some(AssemblerToken {  token_type: AssemblerTokenType::Dw, line  })
            ) => self.parse_two_word_definition(label, line),

            (
                AssemblerToken {  token_type: AssemblerTokenType::LabelToken(ref label), line: _  },
                Some(AssemblerToken {  token_type: AssemblerTokenType::Db, line })
            ) => self.parse_word_definition(label, line),
            (
                AssemblerToken {  token_type: AssemblerTokenType::InstructionCode(instruction), line  },
                ref next,
            ) =>
                self.parse_instruction(instruction, &next.clone().map(|t| t.token_type), line.clone()),
            (t, _) => {
                Err(Error::from(AssemblerError::UndefinedError { line: t.line }))
            },
        }?;
    ...
    fn parse_operation(&mut self, line: usize) -> Result<OperationExpression, Error> {
        ...
        match next {
            Some(AssemblerToken { token_type: AssemblerTokenType::Or, line }) => {
                self.source.next();
                let right_side = self.parse_operation(line)?;
                Ok(OperationExpression::Or(Box::new(left_side), Box::new(right_side)))
            },
            Some(AssemblerToken { token_type: AssemblerTokenType::Xor, line }) => {
                self.source.next();
                let right_side = self.parse_operation(line)?;
    ...
    fn parse_not_operation(&mut self, line: usize) -> Result<OperationExpression, Error> {
        let next = self.source.peek().map(|t| t.clone());
        match next {
            Some(AssemblerToken { token_type: AssemblerTokenType::Not, line }) => {
                self.source.next();
                let right_side = self.parse_sum_operations(line.clone())?;
                Ok(OperationExpression::Not(Box::new(right_side)))
            },
            Some(AssemblerToken { token_type: _, line }) => self.parse_sum_operations(line),
            None => Err(Error::from(AssemblerError::UnexpectedEndOfExpression { line }))
        }
    }
    ...
    fn parse_group(&mut self, line: usize) -> Result<OperationExpression, Error> {
        ...
        match next {
            Some(AssemblerToken { token_type: AssemblerTokenType::LeftParen, line }) => {
                self.source.next();
                let op = self.parse_operation(line.clone())?;
                self.consume(AssemblerTokenType::RightParen, line.clone())?;
                Ok(OperationExpression::Group(Box::new(op)))
            },
            Some(AssemblerToken { token_type: _, line }) => {
    ...
    fn parse_instruction(
        ...
        match (instruction, next) {
            ...
            (InstructionCode::Adc, _) =>
                Err(Error::from(AssemblerError::InvalidInstructionArgument { line })),
            (InstructionCode::Add,
                &Some(AssemblerTokenType::DataStore(l@Location::Register { register: RegisterType::A }))) |
            (InstructionCode::Add,
                &Some(AssemblerTokenType::DataStore(l@Location::Register { register: RegisterType::B }))) |
            ...
            (InstructionCode::Call, _) => self.parse_two_word_instruction(InstructionCode::Call, line),
            ...
            (InstructionCode::Cmp, _) =>
                Err(Error::from(AssemblerError::InvalidInstructionArgument { line })),
            (InstructionCode::Cnc, _) => self.parse_two_word_instruction(InstructionCode::Cnc, line),
            (InstructionCode::Cnz, _) => self.parse_two_word_instruction(InstructionCode::Cnz, line),
            (InstructionCode::Cp, _) => self.parse_two_word_instruction(InstructionCode::Cp, line),
            (InstructionCode::Cpe, _) => self.parse_two_word_instruction(InstructionCode::Cpe, line),
            (InstructionCode::Cpi, _) => self.parse_word_instruction(InstructionCode::Cpi, line),
            (InstructionCode::Cpo, _) => self.parse_two_word_instruction(InstructionCode::Cpo, line),
            ...
            (InstructionCode::Jmp, _) => self.parse_two_word_instruction(InstructionCode::Jmp, line),
            (InstructionCode::Jnc, _) => self.parse_two_word_instruction(InstructionCode::Jnc, line),
            (InstructionCode::Jnz, _) => self.parse_two_word_instruction(InstructionCode::Jnz, line),
            (InstructionCode::Jp, _) => self.parse_two_word_instruction(InstructionCode::Jp, line),
            (InstructionCode::Jpe, _) => self.parse_two_word_instruction(InstructionCode::Jpe, line),
            (InstructionCode::Jpo, _) => self.parse_two_word_instruction(InstructionCode::Jpo, line),
            (InstructionCode::Jz, _) => self.parse_two_word_instruction(InstructionCode::Jz, line),
            (InstructionCode::Lda, _) => self.parse_two_word_instruction(InstructionCode::Lda, line),
            ...
            (InstructionCode::Ldax, _) =>
                Err(Error::from(AssemblerError::InvalidInstructionArgument { line })),
            (InstructionCode::Lhld, _) => self.parse_two_word_instruction(InstructionCode::Lhld, line),
            (InstructionCode::Lxi,
                &Some(AssemblerTokenType::DataStore(l@Location::Register { register: RegisterType::B }))) |
            (InstructionCode::Lxi,
                &Some(AssemblerTokenType::DataStore(l@Location::Register { register: RegisterType::D }))) |
            ...
            (InstructionCode::Mov,
                ...
                match self.source.peek().map(|v| v.clone().token_type) {
                    ...
                    _ => Err(Error::from(AssemblerError::InvalidInstructionArgument { line }))
                }
            },
            ...
            (InstructionCode::Sbi, _) =>
                self.parse_word_instruction(InstructionCode::Sbi, line),
            (InstructionCode::Shld, _) => self.parse_two_word_instruction(InstructionCode::Shld, line),
            (InstructionCode::Sphl, _) =>
                Ok(Statement::InstructionExprStmt(Instruction(InstructionCode::Sphl, None, None))),
            (InstructionCode::Sta, _) => self.parse_two_word_instruction(InstructionCode::Sta, line),
