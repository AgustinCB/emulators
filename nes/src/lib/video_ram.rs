...
impl VideoRam {
    pub(crate) fn new(ram: Rc<RefCell<Ram>>) -> VideoRam {
        VideoRam {
            register2000: ram.borrow().io_registers[0].clone(),
            register2001: ram.borrow().io_registers[1].clone(),
            register2002: ram.borrow().io_registers[2].clone(),
            register2003: ram.borrow().io_registers[3].clone(),
            register2004: ram.borrow().io_registers[4].clone(),
            register2005: ram.borrow().io_registers[5].clone(),
            register2006: ram.borrow().io_registers[6].clone(),
            register2007: ram.borrow().io_registers[7].clone(),
            register4014: ram.borrow().io_registers[28].clone(),
...
mod tests {
    ...
    fn it_should_map_to_the_correct_bits_in_ram() {
        ...
        m.set(0x2007, 0x49);
        m.set(0x4014, 0x41);
        let video_ram = VideoRam::new(Rc::new(RefCell::new(m)));
        ...
        assert_eq!(video_ram.register2007.borrow().current, 0x49);
    ...
    fn it_should_get_from_pattern_tables() {
        ...
        let mut video_ram = VideoRam::new(Rc::new(RefCell::new(m)));
